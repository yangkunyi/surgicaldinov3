...
def adapt_ckpt_for_module(module: torch.nn.Module, ckpt: dict):
    ...
    corresponding param in `module` is a DTensor (mesh/placements match).
    """
    ...
    for k, v in ckpt.items():
        ...
        # If target is DTensor but checkpoint is not, wrap it with the same mesh/placements
        if isinstance(p, DTensor) and not isinstance(v, DTensor):
            # Move the dense tensor to the same device used by the DTensor's local shard
            local_device = p.to_local().device
            out[k] = distribute_tensor(
                v.to(local_device),
                device_mesh=p.device_mesh,
                placements=p.placements,
            )
        # If target is dense but checkpoint is DTensor, convert to local dense
        elif not isinstance(p, DTensor) and isinstance(v, DTensor):
            out[k] = v.to_local()
        else:
            out[k] = v
    return out
