...
try:
    import wandb  # type: ignore[assignment]
...
d
    ...
    if use_wandb:
        ...
        # Derive run name purely from config. If not set, let wandb generate
        # its own default name by omitting the "name" argument.
        run_name_cfg = wandb_cfg.run_name or ""
        run_name = run_name_cfg or None
        ...
        if tags:
            wandb_init_kwargs["tags"] = list(tags)
        if group is not None:
            wandb_init_kwargs["group"] = group

        wandb_run = wandb.init(**wandb_init_kwargs)
        logger.info("Initialized Weights & Biases run: %s", wandb_run.name)
    elif wandb is None and wandb_cfg.enabled and distributed.is_main_process():
        logger.info("wandb is not installed; skipping Weights & Biases logging although wandb.enabled is True.")
    # Optimizer
    ...
    for data in metric_logger.log_every(
        ...
        # Forward metrics to Weights & Biases from the main process only.
        if wandb_run is not None and distributed.is_main_process():
            ...
            # total_loss is a scalar tensor after all-reduce.
            if isinstance(total_loss, torch.Tensor):
                wandb_log["total_loss"] = float(total_loss.item())
            else:
                wandb_log["total_loss"] = float(total_loss)
    ...
    if wandb_run is not None and distributed.is_main_process():
        # Explicitly finish the run to make sure all metrics are flushed.
        wandb_run.finish()
